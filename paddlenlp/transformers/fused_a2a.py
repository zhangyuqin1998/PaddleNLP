...
def FusedDispatch_forward(x, token_indices, token_probs, num_experts, group):
...
def FusedDispatch_backward(group, handle, grad_output, grad_token_probs):
...
def FusedCombine_forward(x, group, states):
...
def FusedCombine_backward(group, handle, grad_output):
...
class FusedDispatch(PyLayer):
    ...
    @staticmethod
    def forward(ctx, x, token_indices, token_probs, num_experts, group, async_finish=False, previous_event=None):
        """Forward pass of fused dispatch."""
        # Calculate layout before actual dispatch
        buffer = get_buffer(group, get_hidden_bytes(x))
        (
...
class FusedCombine(PyLayer):
    ...
    @staticmethod
    def forward(ctx, x, group, states, async_finish=False, previous_event=None):
        """Forward pass of fused combine."""
        handle = states["handle"]
        buffer = get_buffer(group, get_hidden_bytes(x))
        combined_x, _, event = buffer.combine(
...
if HAVE_DEEP_EP:
    ...
    def fused_dispatch(
        x, token_indices, token_probs, num_experts, group: Group, async_finish=False, previous_event=None
    ):
        ...
        return FusedDispatch.apply(
            x.contiguous(), token_indices, token_probs, num_experts, group, async_finish, previous_event
        )

    def fused_combine(x, group, handle, async_finish=False, previous_event=None):
        ...
        states = dict()
        states["handle"] = handle
        return FusedCombine.apply(x, group, states, async_finish, previous_event)
