...
from paddlenlp.transformers.fused_a2a import (
    FusedCombine_backward,
    FusedCombine_forward,
    FusedDispatch_backward,
    FusedDispatch_forward,
)
...
class DecoderLayerNode(ScheduleNode):
    ...
    def dispatch_forward(self, inputs):
        ...
        with paddle.no_grad():
            hidden_states, dispatched_probs, states = FusedDispatch_forward(
                hidden_states, token_indices, token_probs, 8, self.moe_group
            )  # TODO: 8改成num_experts
        tokens_per_expert = states["tokens_per_expert"]
        dispatched_indices = states["dispatched_indices"]
        # tokens_per_expert.stop_gradient = False
        # dispatched_indices.stop_gradient = False
...
class DeepseekV2DecoderLayerPipe(DeepseekV2DecoderLayer):
    ...
    def self_attn_and_gate_compute(self, args):
        ...
        intermediate_hidden_states, token_indices, token_probs = self.mlp.pre_dispatch_compute(
    ...
    def dispatch_comm(self, input):
        ...
        (
            ...
            dispatched_indices,
            dispatched_probs,
        ) = self.mlp.dispatch_comm(intermediate_hidden_states, token_indices, token_probs)
        return (
            inputs_embeds_mtp,
            hidden_states,
            residual,
    ...
    def mlp_compute(self, input):
        ...
        expert_output = self.mlp.mlp_compute(
            intermediate_hidden_states, tokens_per_expert, dispatched_indices, dispatched_probs
        )
        return (
            inputs_embeds_mtp,
            hidden_states,
